(function (domGlobals) {
    "use strict";
    const reg = /([A-Z][0-9]{2}.[0-9]{2}.[0-9]{4}\b)|([0-9]{9}\/[0-9]\/[a-z][0-9]\b)|([0-9]{9}\/[0-9]\b)|([0-9]+HLAR[0-9]{3,}\b)/gi;

    function matchAll (editor) {
        ...
        function _loopThroughChildren(el, cb) {
            let children = $(el).contents();
            children.each((c, ch) => {
                // check if it is a non editable element
                if ( $(ch).hasClass("mwt-ws-non-editable") || $(ch).hasClass("mwt-non-editable") ) {
                    return;
                }

                // if child is text node call callback
                if ( ch.nodeType === 3) {
                    cb(ch);
                }

                // loop through children
                _loopThroughChildren(ch, cb);
            });

            return $(el);
        }

        // go through children and add data-attr if text must be replaced with template call
        $content = _loopThroughChildren($content, (textNode) => {
            let txt = $(textNode).text();
            let test;
            while( test = reg.exec(txt) ) {
                if ( $(textNode).parent()[0].hasAttribute('data-ws-need-2-replace')) {
                    let array = JSON.parse($(textNode).parent().attr('data-ws-need-2-replace')).array;
                    array.push(test[0]);
                    $(textNode).parent().attr('data-ws-need-2-replace', JSON.stringify({array: array}));
                } else {
                    let array = [];
                    array.push(test[0]);
                    $(textNode).parent().attr('data-ws-need-2-replace', JSON.stringify({array: array}));
        ...
        $content = _loopThroughChildren($content, (textNode) => {
            // check if parent has the attr
            if ( $(textNode).parent()[0].hasAttribute('data-ws-need-2-replace') ) {
                let array = $(textNode).parent().attr('data-ws-need-2-replace');
                let text = $(textNode).text();
                array = JSON.parse(array).array;
                // loop through matched strings
                for ( let i = 0; i < array.length; i++ ) {
                    let template = createLinkTemplate(array[i]);
                    // replace matched string with template
                    text = text.replace(array[i], template);
                }
                // replace original text with converted
                $(textNode)[0].data = text;
                $(textNode)[0].nodeValue = text;
                $(textNode)[0].textContent = text;
                $(textNode).parent().removeAttr('data-ws-need-2-replace');
            }
        });

        tContent = $content.html();

        var textObject = {text: tContent};
        $(document).trigger('TinyMCEBeforeWikiToHtml', [textObject]);
        tContent = textObject.text;
        ...
        // add undo action and set content
        editor.undoManager.transact(function() {
            editor.focus();
            editor.selection.setContent(tContent, args);
    ...
    /**
     * creates wiki text for the template
     * @param id
     * @returns {string}
     */
    ...
    var find$1 = function (pattern, sections, currentSearchState) {
        return bind(sections, function (section) {
            var elements = section.elements;
            var content = map(elements, get).join('');
            var positions = find(content, pattern, section.sOffset, content.length - section.fOffset, currentSearchState);
    ...
    var findAndMark = function (dom, pattern, node, replacementNode, currentSearchState) {
        var textSections = fromNode(dom, node);
        var matches = find$1(pattern, textSections, currentSearchState);
    ...
    var markAllMatches = function (editor, currentSearchState, pattern, inSelection) {
        ...
        if (inSelection) {
            ...
            return findAndMark(editor.dom, pattern, node, marker, currentSearchState);
        }
    };
    ...
    var replace = function (editor, currentSearchState, text, forward, all) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (all || matchIndex === searchState.index) {
                var link = nodes[i].firstChild.nodeValue;
                text = createLinkTemplate(link);
                if (text.length) {
    ...
    var open = function (editor, currentSearchState) {
        ...
        var doFind = function (api) {
            ...
            if ( !data.findtext ) data.findtext = 'test';
            ...
            onAction: function (api, details) {
                ...
                switch (details.name) {
                    case 'find':
                        ...
                        break;
                    case 'replace':
                        if (!replace(editor, currentSearchState, "test")) {
                            reset(api);
                        } else {
                            updateButtonStates(api);
                        }
                    ...
                    case 'replaceall':
                        // _matchAll(editor);
                        replace(editor, currentSearchState, 'test', true, true);
                        reset(api);
                        break;
                    case 'prev':
                        prev(editor, currentSearchState);
    ...
    function Plugin () {
        global.add('wsrecoglink', function (editor) {
