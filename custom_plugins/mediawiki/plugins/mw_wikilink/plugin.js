...
var wikilink = function (editor) {
	...
	var open = function ( editor ) {
      ...
      var nwz = selectedNode.closest('.newwin');
		...
		var dialogBody = function ( initialData ) {
          ...
          {text: "New page in this Hub" , value: 'mwt-nonEditable mwt-wikiMagic mwt-newlink mwt-internallink'},
			...
			var linkCtrl = {
				name: 'href',
				type: 'input',
        placeholder: 'Type the name of the page to link to',
...
{text: "Open in the same window", value: 'mwt-target-self'},
        ...
        if(obj.class == 'mwt-nonEditable mwt-wikiMagic mwt-internallink'){
          document.querySelector('[title="OK"]').classList.add('disabled');
            ...
            if(!autocompletebox){
              ...
              autocompletebox.classList.add('icon-list');
              autocompletebox.classList.add('dropdown-menu');
              autocompletebox.classList.add('d-block');
              ...
              fakeinput.classList.add('tox-textfield')
              fakeinput.value = formGroup.querySelector('input').value;
              formGroup.appendChild(fakeinput)
              fakeinput.focus()
              ...
              fakeinput.addEventListener('input', function(e){
               ...
               var params = {
  		 action: 'ask',
  		 query: '[[Class::Page]][[Title::~*'+fakeinput.value+'*]]|?Title|?Home.Name|?Page ID|sort=es.score|limit=20|order=desc',
  		 format: 'json'
  		},
  		...
  		api.postWithToken( 'csrf', params ).done( function ( data ) {
            ...
            autocompletebox.innerHTML = "";
            var more = false;
            Object.entries(data.query.results).forEach(function(value){
              more = true;
              var opt = document.createElement('li');
              opt.classList.add('dropdown-item')
              var nsp = "";
              if(value[1].printouts.Name && value[1].printouts.Name[0] && value[1].printouts.Name[0].fulltext){
                nsp = value[1].printouts.Name[0].fulltext;
              }
              var tx = nsp+':'+value[1].printouts.Title[0].fulltext;
              var regex = new RegExp( fakeinput.value , 'gmi' );
              opt.innerHTML = tx.replace(regex , '<b>'+fakeinput.value+'</b>');
              opt.setAttribute('data-value', value[1].printouts['Page ID'][0]);
              autocompletebox.appendChild(opt);
              console.log(value)
            })

            if(!more){
               var opt = document.createElement('li');
              opt.innerText = "No results";
              opt.setAttribute('data-value', "");
              autocompletebox.appendChild(opt);
            }
  ...
  fakeinput.classList.remove('d-none');
  fakeinput.focus();
  formGroup.querySelector('input').classList.add('d-none');
      ...
      if(changed.name == 'class'){
        ...
        if(obj.class != 'mwt-nonEditable mwt-wikiMagic mwt-internallink'){
          ...
          var  fakeinput =  document.querySelector('#fakeinput');
          if(fakeinput){
          fakeinput.classList.add('d-none')
          formGroup.querySelector('input').classList.remove('d-none');
        ...
        if (data.target && data.target == "mwt-target-blank") {
